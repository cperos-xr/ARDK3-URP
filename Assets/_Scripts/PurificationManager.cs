...
public class PurificationManager : MonoBehaviour
{
    ...
    private void InitializePurificationEntity(SO_CorruptEntity corruptEntity)
    {
        ...
        corruptEntityImage.sprite = currentPurificationEntity.corruptEntity.corruptedStateSprite;
    ...
    void BeginPurification()
    {
        ...
        InitializePurificationEntity(currentPurificationEntity.corruptEntity);
        selectedEssenceMaterials.Clear();
        CopyPlayerEssencePouch();
        Debug.Log("Begining the purification of " + currentPurificationEntity.corruptEntity.corruptEntityName);
    ...
    public void AttemptPurification()
    {
        ...
        if (currentPurificationEntity.currentCorruptionLevel <= 0)
        {
            ...
            corruptEntityImage.sprite = currentPurificationEntity.corruptEntity.healedStateSprite;
            ...
            if (currentPurificationEntity.corruptEntity.ItemsDroppedUponPurification.Count > 0)
            {
                foreach(ItemDrop itemDrop in currentPurificationEntity.corruptEntity.ItemsDroppedUponPurification)
                {
                    float rand = Random.Range(0, 1);
                    if (rand < itemDrop.dropRate)
                    {
                        //Give item to player
                        itemManager.AddItemToPlayerInventory(itemDrop.item, currentPurificationEntity.corruptEntity); // not happening?
                    }
                }
            }

            if(currentPurificationEntity.corruptEntity.InteractionToCompleteUponPurification != null)
            {
                // make the current interaction the interaction to complete up purification
                InteractionManager.Instance.UpdateInteraction(currentPurificationEntity.corruptEntity, currentPurificationEntity.corruptEntity.InteractionToCompleteUponPurification);
                // do the interaction
                InteractionManager.Instance.HandleEntityInteraction(currentPurificationEntity.corruptEntity);
            }
        }
    ...
    private float CalculatePurificationPoints()
    {
        ...
        {
            foreach (EssenceEffectiveness corruptedEntityEffectiveEssence in currentPurificationEntity.corruptEntity.essenceEffectivenessList)
            {
        ...
        for (int i = 0; i < selectedEssenceMaterials.Count; i++)
        {
            for (int j = i + 1; j < selectedEssenceMaterials.Count; j++)
            {
                ...
                foreach (CombinationMultiplier combinationMultiplier in currentPurificationEntity.corruptEntity.combinationMultipliersList)
                {
    ...
    PurificationEntity CreateNewPurificationEntity(SO_CorruptEntity corruptEntity)
    {
        ...
        purificationEntity.corruptEntity = corruptEntity;
